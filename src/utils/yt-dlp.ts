...
/** Resolve cookies file: env path first, then cookies.txt in project root. */
...
/** Format --js-runtimes value (node + path for VPS/Pterodactyl). */
export function formatJsRuntimes(value: string): string | null {
...
/** Global yt-dlp flags applied to every invocation (cookies, JS runtime, EJS). */
...
export function logYtDlpSetupStatus(): void {
	const cookiesPath = resolveCookiesPath();
	if (cookiesPath) {
		logger.info(`yt-dlp cookies loaded: ${cookiesPath}`);
	} else {
		logger.warn(
			"yt-dlp: no cookies.txt found. YouTube on VPS may fail with 'Sign in to confirm you are not a bot'. " +
...
function buildArgs(url: string, options: Partial<YTFlags>): string[] {
	const optArgs: string[] = [...getBaseYtDlpArgs()];
...
/** Get a direct stream URL (no download) for any site yt-dlp supports. */
export async function getStreamUrl(url: string, maxHeight?: number): Promise<string | null> {
